...
//
// IMPORTANT: parseWorkoutText()'s pre-flight warnings (missing-blank-line,
// prose-duration-pattern) are *heuristics*, not a confirmed causal model.
// Follow-up testing against the real API found a doc with zero blank lines
// that parsed fine, and prose with duration-like tokens ("200mg 1h", "30
// min", "3'") that did NOT produce phantom steps — so these patterns
// correlate with known failures but don't reliably predict them, in either
// direction. Treat their absence as "nothing obviously wrong", not "safe".
// verifyWorkoutDoc() — which diffs against what Intervals actually stored —
// is the only authoritative signal; see the event-123652836 fixture in
// workout-text.test.js for a confirmed-broken case the pre-flight heuristics
// don't explain.
  ...
  lines.forEach((rawLine, idx) => {
    ...
    if (DURATION_PATTERN.test(line)) {
      warnings.push({
        ...
        line: lineNo,
        message: `Line ${lineNo}: "${line}" reads like prose but contains a duration-like token; this pattern has produced phantom steps before, though not reliably (heuristic — verify against the response, not just this warning).`,
      })
    } else if (sawStepsInCurrentBlock) {
      warnings.push({
        type: "missing-blank-line-before-block",
        line: lineNo,
        message: `Line ${lineNo}: block header "${line}" isn't preceded by a blank line separating it from the previous block — this pattern has caused a lost power target before, though not reliably (heuristic — verify against the response, not just this warning).`,
      })
    }
    repStack = []
    sawStepsInCurrentBlock = false
...
export function verifyWorkoutDoc(expected, workoutDoc) {
  ...
  const issues = []
  const countMatches = actualSteps.length === expected.steps.length
  ...
  // Comparing step[i] to step[i] only makes sense once the counts line up —
  // otherwise everything after the first drop/insert is offset by one and
  // produces cascading false positives (one real bug reported as several).
  if (countMatches) {
