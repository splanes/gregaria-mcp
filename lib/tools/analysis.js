// Aggregate analysis tools: fan-out to several client calls + JS-side
// aggregation (counts, sums, averages, weekly load buckets, rule-based
// recommendations), returning JSON instead of Markdown.
...
// Average sleep hours. The API exposes sleepSecs (seconds), not "sleep".
...
function trainingLoadRecommendations({ weeklyLoad, plannedWorkouts, completedActivities, avgFatigue, avgSleepHours }) {
  ...
  if (weeklyLoad > 900) recommendations.push("Consider a recovery week with reduced volume.")
  else if (weeklyLoad < 300) recommendations.push("Gradually increase training load.")

  if (plannedWorkouts > 0 && completedActivities < plannedWorkouts * 0.7) {
    recommendations.push("Review the training plan — adherence is low, consider scaling back what's planned.")
  } else if (plannedWorkouts > 0 && completedActivities > plannedWorkouts) {
    recommendations.push("Good plan adherence — make sure to get adequate recovery.")
  }

  if (avgFatigue > 7) recommendations.push("High fatigue detected — prioritize sleep and active recovery.")
  else if (avgSleepHours > 0 && avgSleepHours < 7) recommendations.push("Insufficient sleep — prioritize recovery.")

  if (!recommendations.length) {
    recommendations.push("Training load looks balanced — keep up the current approach.")
  }
...
export function registerAnalysisTools(server) {
  server.tool(
    "analyze_activity",
    "Comprehensive analysis of a single activity: detail + intervals + streams summary + latest messages.",
  ...
  server.tool(
    "analyze_progress",
    "Athlete's progress over a period: activities by type, total distance/time/load, wellness averages, power curve.",
  ...
  server.tool(
    ...
    "Training load and recommendations: total/weekly load, plan vs. execution, recovery status.",
